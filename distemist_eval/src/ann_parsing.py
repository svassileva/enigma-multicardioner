...
def main(datapath, relevant_labels, codes_path):
    # Load
    valid_codes = set(map(lambda k: k.split('\t')[0], open(codes_path).readlines())) if codes_path != '' else set()
    df = pd.read_csv(datapath, sep='\t', header=0, quoting=csv.QUOTE_NONE, keep_default_na=False, dtype=str)

    # Check column names are correct 
    if ','.join(df.columns) == ','.join(['filename','ann_id','label','start_span','end_span','text']):
        print("\nAccording to file headers, you are on subtask ner, GS file")
    elif ','.join(df.columns) == ','.join(['filename','label','start_span','end_span','text']):
        print("\nAccording to file headers, you are on subtask ner, predictions file")
    elif ','.join(df.columns) == ','.join(['filename','ann_id','label','start_span','end_span',
                                           'text', 'code']):
        print("\nAccording to file headers, you are on subtask norm, predictions file")
    elif ','.join(df.columns) == ','.join(['filename','label','start_span','end_span',
                                           'text', 'code']):
        print("\nAccording to file headers, you are on subtask norm, predictions file")
    elif ','.join(df.columns) == ','.join(['filename','ann_id','label','start_span','end_span',
                                           'text', 'code', 'semantic_rel']):
    ...
    df_ok['offset'] = df_ok['start_span'].astype(str) + ' ' + df_ok['end_span'].astype(str)
